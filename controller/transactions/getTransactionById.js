...
router.get("/:id", async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "budget or user id is required is required" });
  }
  ...
  try {
    ...
    // If no transactions found with userId, attempt to find with budget_Id
    if (transactions.length === 0) {
      transactions = await Transaction.find({ budget_Id: id })
        .sort({ createdAt: -1 }) // Sort by creation date descending
        .skip(skip)
        .limit(limit);
    }
